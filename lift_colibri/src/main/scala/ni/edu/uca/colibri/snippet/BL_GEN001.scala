 ...
 */ 
   ...
   */
		...
		list.map(a => 
        	{     
        		...
        		if (a.elements(0).checked == true){
					...
					S.redirectTo("/Generate/GEN002")
        		}
		})
	}
	
	def exitBtn()={
		S.redirectTo("/Generate/GEN001")
	}
	...
	var list = {
      ...
      val search:List[DAO_OPERATION_001] = {
       
             mode match {
                // Ref
                case "1" =>DAO_OPERATION_001.findAll(By(DAO_OPERATION_001.delete_fg, 0), By(DAO_OPERATION_001.status, 0))	
                // Add
                case "2" => {	Session.operation_idVar(0)
                				Session.modeVar(mode.toInt)
                				S.redirectTo("/Generate/GEN002ADD")  
                }
                // Del
                case "4" => DAO_OPERATION_001.getOperationByStatus(Seq(0)) 
                // Undo App
 ...
 */ 
    def transaction():NodeSeq ={
		<table>
		  <tr>
		      <td><GEN001:Next><button>Next</button></GEN001:Next></td>
		      <td><GEN001:Exit><button>Exit</button></GEN001:Exit></td>
		  </tr>
		</table>
    }
   ...
   */ 
	 def display(form: NodeSeq) ={
       { mode = form.toString }
     <lift:BL_GEN001.execute form="post" multipart="true">	
	       {table}  
	       {transaction}
     </lift:BL_GEN001.execute>
	 }
